...
public static class pdfUtility
{
    ...
    public static List<StatementHeader> ExtractStatementsFromPdf(string path)
    {
        ...
        Debug.WriteLineIf(deb, "Creating PDF Reader");
        ...
        using (PdfReader reader = new PdfReader(path))
        {
            ...
            Debug.WriteLineIf(deb, "Pages Found : " + reader.NumberOfPages);

            for (int i = 1; i <= reader.NumberOfPages; i++)
            {
                Debug.WriteLineIf(deb,  "Page " + i );
                Debug.WriteLineIf(deb, "-------------------------------------");
                string thePage = "~" + PdfTextExtractor.GetTextFromPage(reader, i);   // add leading newLine 
                if(thePage.Contains("MoneyMarket Call Account"))
                {
                    throw new Exception("Invalid Statment Type - <br/><b>'MoneyMarket Call Account'</b><br/> Statements are not supported.<br/>");
                }
                ...
                try
                {
                    try
                    {
                        Debug.WriteLineIf(deb, "Trying to extract Statement Header From Page ");
    ...
    public static void genPDF(string pathToFile, decimal interestPaid, decimal balanceAtEnd, DateTime startDate, DateTime endDate, List<StatementHeader> stmnts, string region, string name, string num)
    {
        try
        {
            ...
            catch(Exception coNameException)
            {
                throw new Exception("Company Name Not Found!", coNameException);
            }
            cb.SetColorStroke(new BaseColor(System.Drawing.Color.DarkBlue));
            cb.EndText();
        ...
        catch (Exception e)
        {
            ...
            throw new Exception("PDFGenerationExceptoion", e);
        }
    }

    public static void DebugWrite(string mode, string logLine, string debugFile)
    {
        if (mode != "none")
        {
            bool doHeader = false;
            ...
            StreamWriter sw = File.AppendText(debugFile);
            if (doHeader)
            {
                sw.WriteLine("Start of Debug File - " + DateTime.Now.ToShortDateString() + " " + DateTime.Now.ToShortTimeString());
                sw.WriteLine("");
            }
